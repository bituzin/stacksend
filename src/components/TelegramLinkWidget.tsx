...
export function TelegramLinkWidget({ walletAddress }: TelegramLinkWidgetProps) {
    ...
    return (
        <div className="bg-white/80 dark:bg-gradient-to-br dark:from-gray-800/50 dark:to-gray-900/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700/50 rounded-xl p-6 shadow-xl">
            <div className="flex items-start justify-between mb-4">
                <div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1">Telegram Notifications</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Get instant alerts when you receive transfers</p>
                </div>
                {isTelegramLinked && (
                    <button
                    ...
                    <div className="flex items-center justify-between p-4 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 rounded-lg">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                <div className="w-2 h-2 bg-green-500 dark:bg-green-500 rounded-full animate-pulse" />
                                <span className="text-green-700 dark:text-green-400 font-semibold">Connected</span>
                            </div>
                            {user.telegram_username && (
                                <p className="text-sm text-gray-600 dark:text-gray-400">@{user.telegram_username}</p>
                            )}
                        </div>
                        <div className="text-right">
                            <span className={`text-sm ${user.notification_enabled ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-500'}`}>
                ...
                <div className="space-y-4">
                    <div className="p-4 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-lg">
                        ...
                        <ol className="text-sm text-gray-700 dark:text-gray-300 space-y-2 ml-4 list-decimal">
                            ...
                            <li>Send <code className="px-2 py-1 bg-gray-200 dark:bg-gray-800 text-blue-600 dark:text-blue-400 rounded">/ start</code></li>
