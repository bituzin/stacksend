...
export function ActivityFeed({ walletAddress }: ActivityFeedProps) {
    ...
    if (!walletAddress) {
        return (
            <div className="bg-white/80 dark:bg-gradient-to-br dark:from-gray-800/50 dark:to-gray-900/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700/50 rounded-xl p-8 text-center">
                <p className="text-gray-600 dark:text-gray-400">Connect your wallet to view transfer history</p>
            </div>
        );
    }

    return (
        <div className="bg-white/80 dark:bg-gradient-to-br dark:from-gray-800/50 dark:to-gray-900/50 backdrop-blur-sm border border-gray-200 dark:border-gray-700/50 rounded-xl shadow-xl overflow-hidden">
            {/* Header */}
            <div className="p-6 border-b border-gray-200 dark:border-gray-700/50">
                <div className="flex items-center justify-between">
                    <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-1">Transfer Activity</h3>
                        <p className="text-sm text-gray-600 dark:text-gray-400">Your recent StackSend transactions</p>
                    </div>
                    <button
                        onClick={loadActivity}
                        disabled={loading}
                        className="p-2 rounded-lg bg-gray-100 dark:bg-gray-700/50 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 transition-all disabled:opacity-50"
                    >
            ...
            {/* Error State */}
            {error && (
                <div className="p-4 m-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-lg text-red-600 dark:text-red-400 text-sm">
                    {error}
                </div>
            )}
            ...
            {!loading && activity.length === 0 && !error && (
                <div className="p-12 text-center">
                    <div className="w-16 h-16 bg-gray-200 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-4">
                        <ArrowUpRight className="w-8 h-8 text-gray-400 dark:text-gray-600" />
                    </div>
                    <p className="text-gray-600 dark:text-gray-400 mb-2">No transfer activity yet</p>
                    <p className="text-sm text-gray-500 dark:text-gray-500">Your received and sent transfers will appear here</p>
                </div>
            )}
                                    ...
                                    <div className="flex items-start justify-between mb-2">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                                ...
                                                <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded text-xs">
            ...
            {activity.length >= 50 && (
                <div className="p-4 border-t border-gray-200 dark:border-gray-700/50 text-center">
                    <p className="text-sm text-gray-600 dark:text-gray-400">Showing most recent 50 transfers</p>
                </div>
            )}
        </div>
    );
